...
namespace DynastyEngine
{
    ...
    {
    public:
        template<class T> inline static void hashCombine(size_t &seed, T const &v) 
        {
            seed ^= std::hash<T>()(v) + 0x9e3779b9u + (seed << 6u) + (seed >> 2u);
        }

        // 采用了MurmurHash3算法。它接受一个种子值（seed）、一个密钥（key）和一个字数（wordCount）作为输入，然后处理密钥以生成一个哈希值
        ...
        template<typename T> inline static void hashCombineMurmur(size_t &seed, const T &key) 
        {
