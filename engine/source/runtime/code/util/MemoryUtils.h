...
namespace DynastyEngine
{
    ...
    public:
        ...
        template<typename T> static std::shared_ptr<T> makeAlignedBuffer(size_t elemCnt) 
        {
            if (elemCnt == 0) 
            {
                return nullptr;
        ...
        template<typename T> static std::shared_ptr<T> makeBuffer(size_t elemCnt, const uint8_t *data = nullptr) 
        {
            if (elemCnt == 0) 
            {
                return nullptr;
