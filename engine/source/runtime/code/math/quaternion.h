...
namespace DynastyEngine
{
    ...
    public:
        ...
        explicit Quaternion(const glm::mat3& rot) { this->fromRotationMatrix(rot); }
        /// Construct a quaternion from an angle/axis
        Quaternion(const Radian& angle, const glm::vec3& axis) { this->fromAngleAxis(angle, axis); }
        ...
        void fromRotationMatrix(const glm::mat3& rotation);
        void toRotationMatrix(glm::mat3 & rotation) const;
        void toRotationMatrix(glm::mat4 & rotation) const;

        void fromAngleAxis(const Radian& angle, const glm::vec3& axis);

        static Quaternion getQuaternionFromAngleAxis(const Radian& angle, const glm::vec3& axis);

        void fromDirection(const glm::vec3& direction, const glm::vec3& up_direction);

        static Quaternion getQuaternionFromDirection(const glm::vec3& direction, const glm::vec3& up_direction);

        void toAngleAxis(Radian & angle, glm::vec3 & axis) const;

        /** Constructs the quaternion using 3 axes, the axes are assumed to be orthonormal
            @See FromAxes
        */
        void fromAxes(const glm::vec3& x_axis, const glm::vec3& y_axis, const glm::vec3& z_axis);
        /** Gets the 3 orthonormal axes defining the quaternion. @See FromAxes */
        void toAxes(glm::vec3 & x_axis, glm::vec3 & y_axis, glm::vec3 & z_axis) const;

        /** Returns the X orthonormal axis defining the quaternion. Same as doing
            xAxis = glm::vec3::UNIT_X * this. Also called the local X-axis
        */
        glm::vec3 xAxis() const;

        /** Returns the Y orthonormal axis defining the quaternion. Same as doing
            yAxis = glm::vec3::UNIT_Y * this. Also called the local Y-axis
        */
        glm::vec3 yAxis() const;

        /** Returns the Z orthonormal axis defining the quaternion. Same as doing
            zAxis = glm::vec3::UNIT_Z * this. Also called the local Z-axis
        */
        ...
        float length() const { return Math::sqrt(w * w + x * x + y * y + z * z); }
