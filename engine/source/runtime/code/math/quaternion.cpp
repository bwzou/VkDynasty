...
namespace DynastyEngine
{
    ...
    //-----------------------------------------------------------------------
    void Quaternion::fromRotationMatrix(const glm::mat3& rotation)
    {
    ...
    //-----------------------------------------------------------------------
    void Quaternion::toRotationMatrix(glm::mat3& kRot) const
    {
        float fTx  = x + x;   // 2x
        float fTy  = y + y;   // 2y
        float fTz  = z + z;   // 2z
    ...
    void Quaternion::toRotationMatrix(glm::mat4& kRot) const
    {
        float fTx  = x + x;   // 2x
        float fTy  = y + y;   // 2y
        float fTz  = z + z;   // 2z
    ...
    void Quaternion::fromAngleAxis(const Radian& angle, const glm::vec3& axis)
    {
    ...
    Quaternion Quaternion::getQuaternionFromAngleAxis(const Radian& angle, const glm::vec3& axis)
    {
        Quaternion q;
    ...
    void Quaternion::fromDirection(const glm::vec3& direction, const glm::vec3& up_direction)
    {
        glm::vec3 forward_direction = direction;
        forward_direction.z       = 0.0f;
        forward_direction = glm::normalize(forward_direction);

        glm::vec3 left_direction = glm::cross(up_direction, forward_direction);

        fromAxes(left_direction, -forward_direction, up_direction);
        normalise();
    }

    Quaternion Quaternion::getQuaternionFromDirection(const glm::vec3& direction, const glm::vec3& up_direction)
    {
    ...
    void Quaternion::toAngleAxis(Radian& angle, glm::vec3& axis) const
    {
        // The quaternion representing the rotation is
        //   q = cos(A/2)+sin(A/2)*(x*i+y*j+z*k)
    ...
    //-----------------------------------------------------------------------
    void Quaternion::fromAxes(const glm::vec3& xaxis, const glm::vec3& yaxis, const glm::vec3& zaxis)
    {
        glm::mat3 rot;
    ...
    //-----------------------------------------------------------------------
    void Quaternion::toAxes(glm::vec3& xaxis, glm::vec3& yaxis, glm::vec3& zaxis) const
    {
        glm::mat3 rot;
